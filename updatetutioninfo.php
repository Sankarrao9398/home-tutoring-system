...
<html>
<head>
	<title>Update Tuition Info</title>
...
<?php
 ...
 if (isset($_POST['updatetutioninfo'])) {
	...
	$university_name = $_POST['university']; // Get university name
	...
	try {
		if (($user == $_REQUEST['uid']) && ($utype_db == "teacher")) {
			...
			if ($result4 = $con->query("INSERT INTO tutor (t_id, prefer_sub, class, medium, inst_name, salary, prefer_location) VALUES ('$user', '$sublist', '$classlist', '$mediumlist', '$university_name', '$_POST[sal_range]', '$_POST[location]')")) {
...
<nav>
	...
	<ul class="nav-links">
		...
		<?php 
		if ($utype_db == "teacher") {
			// No additional link for teachers
		} else {
		...
		<div>
			<form action="" method="POST" class="registration">
				...
				if (isset($error_message)) {
					echo '<div class="signup_error_msg" style="color: #A92A2A;">'.$error_message.'</div>';
				} elseif (isset($success_message)) {
					echo '<div class="signup_error_msg" style="color: #A92A2A;">'.$success_message.'</div>';
				}
				echo '<div class="setting-box post-box">
						<h2 style="text-align: center;">Update Tuition Information</h2>';
				?>
				...
				<div class="medium">
					...
					<div class="md">
						<div class="divp35">
							<input <?php $medium1=strstr($medium, "Telugu"); if($medium1 != '') echo 'checked'; ?> type="radio" name="medium_list[]" id="md" value="Telugu"><span>Telugu</span>
						</div>
						<div class="divp35">
							<input <?php $medium1=strstr($medium, "English"); if($medium1 != '') echo 'checked'; ?> type="radio" name="medium_list[]" id="md" value="English"><span>English</span>
						</div>
					</div>
				</div>
				<div class="subject">
